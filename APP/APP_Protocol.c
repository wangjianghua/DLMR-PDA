...
u32 Proto_DL645_Proc(void)
{
    ...
    {                
        if((0 == dl645_frame_stat.Status) || (0 == dl645_frame_stat.C) || (dl645_frame_stat.C & DL645_REPLY_STAT_MASK))
        {
            g_proto_para.recv_result = RECV_RES_INVALID;                            
        }
        else
    ...
    while (DEF_TRUE) {    
        ...
        if(OS_ERR_NONE == err)
        {
            ...
            case GUI_CMD_COMMON:
                ...
                if(CHANNEL_RF == g_rom_para.channel) //射频
                {
