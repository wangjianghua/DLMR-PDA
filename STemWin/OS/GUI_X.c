...
//extern volatile  U32  OSTime;                   /* Current value of system time (in ticks)         */
//#endif
...
*       Global data
*/
#define  OS_TimeMS    OSTime

static OS_EVENT  * g_gui_mutx;
static OS_EVENT  * g_gui_sem;
...
{ 
    ...
    if(OS_ERR_NONE == OSSemQuery(g_gui_mutx, &sem_data))
    {
        if(sem_data.OSCnt == 0)
        {
            OSSemPost(g_gui_mutx);
        }
    }
}
