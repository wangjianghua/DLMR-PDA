...
u8 SD_GetResponse(u8 Response)
{
	...
	while ((SD_ReadWriteByte(0XFF)!=Response)&&Count)Count--;//等待得到准确的回应  	  
...
u8 SD_Init(void)
{
	...
	if(r1==0X01)
	{
		...
		}else//SD V1.x/ MMC	V3
		{
			...
			if(retry==0||SD_SendCmd(CMD16, SD_SECTOR_SIZE, 0X01)!=0)SD_Type=SD_TYPE_ERR;//错误的卡
		}
	}
	SD_DisSelect();//取消片选
	SD_SPI_SpeedHigh();//高速
...
/* FatFs low level driver @华兄 Email: 591881218@qq.com */
...
u8 get_sd_info(void)
{
    ...
    g_sys_control.sd_total_capacity = 0; //总容量，单位KB
    g_sys_control.sd_free_capacity = 0; //剩余容量，单位KB
    ...
    g_sys_control.sd_total_capacity = total_sectors >> 1; //总容量，单位KB
    g_sys_control.sd_free_capacity = free_sectors >> 1; //剩余容量，单位KB
